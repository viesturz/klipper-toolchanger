...
example, if one were to issue a MANUAL_RAIL ... GCODE_AXIS=R command then one could issue commands like G1 X10 Y20
R30 to move the stepper motor. The resulting moves will occur synchronously with the associated toolhead xyz movements.
If the motor is associated with a GCODE_AXIS then one may no longer issue movements using the above MANUAL_RAIL
command - one may unregister the stepper with a MANUAL_RAIL ... GCODE_AXIS= command to resume manual control of the

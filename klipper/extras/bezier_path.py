...
class Bezier:
    ...
    def bezier_curve(self,points, n=1000):
        ...
        polynomial_array = np.array(
            [self._bernstein_poly(i, nPoints-1, t) for i in range(0, nPoints)]
        )
...
class RoundedPath:
    ...
    def _arc(self, c:ControlPoint, p:ControlPoint, n:ControlPoint):
        ...
        np_n = np.array(vn)*c.lin_d     # next tangential point
        np_p = np.array(vp)*c.lin_d     # previous tangential point
        np_c = np.array(c.vec)          # center of curve
        n = math.trunc(math.hypot(*(np_n - np_p))/self.mm_per_arc_segment)      # No of straight points between next and prev
        # logging.info('Bezier arc: n=%d from=[%.3f %.3f %.3f] to=[%.3f %.3f %.3f] c=[%.3f %.3f %.3f]', n, np_p[0], np_p[1], np_p[2],  np_n[0], np_n[1], np_n[2],  np_c[0], np_c[1], np_c[2])
        beziercurve = Bezier().bezier_curve([np_p + np_c, np_c, np_n + np_c], n=n)
        self._g0p(c, _vadd(center, spoke))
        for i,p in enumerate(reversed(beziercurve)):
            # logging.info('Bezier[%d]: %.3f %.3f, %.3f', i, p[0], p[1], p[2])
            self._g0p(c, p)
